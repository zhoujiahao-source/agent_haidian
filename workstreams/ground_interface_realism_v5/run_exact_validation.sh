...
[[ -n "$GH_TOKEN" ]] && gh auth setup-git >/dev/null
...
for token in ['WebGPURenderer','OrbitControls','mergeGeometries','ExtrudeGeometry','InstancedMesh','overpass-api','osmLandscape','canonicalBuildings','pointInPoly','Ground Interface','Not yet canonicalized','ODbL','Staffed safety / governance desk','Test safety buffer','Ordinary + accessible route','Rider / loading pocket']:
...
git clone -q --filter=blob:none --no-checkout https://github.com/zhoujiahao-source/agent_haidian.git /tmp/formal-v5
...
cp -a "/tmp/formal-v5/$PKG" "/tmp/upstream-v5/$PKG"
git -C /tmp/upstream-v5 remote rename origin upstream
git -C /tmp/upstream-v5 remote add origin https://github.com/zhoujiahao-source/agent_haidian.git
...
printf '\n== render and official validation ==\n'
...
printf '\n== freeze twin and save safe validation evidence ==\n'
mkdir -p "$ROOT/validation"
cp /tmp/ground-self.json "$ROOT/validation/self_check_exact.json"
python - <<'PY'
import json
from pathlib import Path
d=json.loads(Path('/tmp/ground-preflight.json').read_text(encoding='utf-8'))
safe={
 'schema_version':'1',
 'evidence_type':'official_participant_preflight_safe_summary',
 'ok':d.get('ok'),
 'submission_dir':d.get('submission_dir'),
 'pr_author':d.get('pr_author'),
 'branch':d.get('branch'),
 'origin_repo':'zhoujiahao-source/agent_haidian',
 'upstream_repo':'open-city-ai/haidian',
 'workspace':d.get('workspace'),
 'base_ref':d.get('base_ref'),
 'outside_scope_files':d.get('outside_scope_files'),
 'package_file_count':d.get('package_file_count'),
 'package_bytes':d.get('package_bytes'),
 'self_check_ok':bool((d.get('self_check') or {}).get('ok')),
 'blockers':d.get('blockers') or [],
 'warnings':d.get('warnings') or [],
 'sanitization_note':'Credential-bearing remote URLs are intentionally not persisted.'
}
Path('workstreams/ground_interface_realism_v5/validation/preflight_exact.json').write_text(json.dumps(safe,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
PY
UPSTREAM_SHA="$LATEST" FORMAL_SHA="$FORMAL_SHA" python - <<'PY'
import json,os,re
from pathlib import Path
root=Path('workstreams/ground_interface_realism_v5'); twin=root/'threejs/keyarea_ground_interface_twin.html'
t=twin.read_text(encoding='utf-8'); t=re.sub(r"DEFAULT_REF='[^']+'",f"DEFAULT_REF='{os.environ['FORMAL_SHA']}'",t,count=1); twin.write_text(t,encoding='utf-8')
report={'schema_version':'1','status':'PASS','validated_upstream_sha':os.environ['UPSTREAM_SHA'],'validated_formal_sha':os.environ['FORMAL_SHA'],'checks':['ground_interface_spec','threejs_node_syntax','bilingual_proposal_patch','official_render_proposal_html','official_self_check_submission','official_participant_preflight'],'spatial_deepening':['ground-floor edge sequence','ordinary accessible route','blue-green comfort','human-help baseline','Human Stop / Human Override contract','test/service separation','AI-off review floor','OSM water/green context','per-building OSM height evidence'],'truth_boundary':['KEY_AREA remains provisional','ground-interface placements are Derived / Not yet canonicalized','section proportions are diagrammatic only','OSM remains candidate existing context','no statutory FAR/height/ownership/road-redline/approved-engineering claim']}
